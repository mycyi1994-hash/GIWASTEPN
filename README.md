...
| 커뮤니티 | 크루 검색·가입/탈퇴(로컬 저장), **파티런 로비**, 인기 게시글, 활동 피드 |
| 아이템 | 착용 스니커즈 히어로, **NFT 민팅(500 SUP, 행운 스탯이 확률 보정)**, 컬렉션 그리드, 강화(실제 SUP 차감), **부스트 상점(에너지 셀·스트릭 실드·XP 부스터 — 실제 구매·적용)** |
...
- **5개 모델**(Apex Runner·Volt Trainer·Trail Blazer·Sprint X·Flow Glide) × **12개 컬러웨이** × **5단계 희귀도**(Common→Legendary)
- 모든 신발은 Canvas 벡터 실루엣으로 렌더링 — 모델별 형태(밑창·토캡·칼라·스트라이프)와 컬러웨이가 실제로 다르게 그려짐
- 민팅 시 효율/행운/착화감 스탯이 희귀도 상한 내에서 랜덤 롤 — 민팅 번호(#0001) 부여
- 효율 = 적립 배율, 착화감 = 에너지 절감(최대 25%), 행운 = 민팅 확률 보정
- 희귀도가 최대 강화 레벨(10~30)과 발광 아우라 강도를 결정
