...
무겁고 격식 있는 다크 럭셔리 대신, **따뜻한 크림 캔버스 + 선셋 코럴**로
"캐주얼하지만 고급스러운" 라이프스타일 앱의 문법을 따릅니다.

| 축 | 규칙 |
|---|---|
| **색** | 크림(`#F9F5EE`) 캔버스 + 선셋 코럴(`#FF6B4A`)·허니(`#EFA92F`). 강조색은 항상 2~3단 그라데이션으로만 쓰고 단색 면으로 칠하지 않는다 |
| **획** | 큰 숫자는 `FontWeight.Light` + 음수 자간(고급감의 뼈대), 제목·라벨은 SemiBold로 친근하게 |
| **면** | 카드는 흰 종이 표면 + 부드러운 이중 그림자. 외곽선 대신 그림자가 면을 띄운다 |

주요 표현 기법

- **선셋 스윕** — 진행 링이 코럴에서 살구로 물들며 차오른다 (`SunsetSweep`)
- **따뜻한 그림자** — CTA·히어로 카드의 그림자에 코럴 기운을 섞어 배경으로 번지게 한다
- **흐르는 광택(sheen)** — SUP 지갑 카드와 주 CTA 위를 하이라이트 띠가 주기적으로 지나간다
- **기요셰(guilloché) 각인** — 지갑 히어로 카드에 동심원 결을 은은하게 새긴다
- **숫자 롤업** — 걸음 수·진행률이 툭 바뀌지 않고 굴러 올라간다 (`animatedInt`, `animatedFloat`)
- **플로팅 독 내비게이션** — 떠 있는 pill 형태의 하단 탭, 활성 탭엔 선셋 점 하나
- **시간대 인사** — 홈 헤더가 아침/오후/노을/밤에 맞춰 캐주얼하게 말을 건다

디자인 토큰과 공용 컴포넌트는 각각 `ui/theme/`와 `ui/components/`에 모여 있어,
색 하나만 바꿔도 화면 전체의 빛의 방향이 함께 움직입니다.

## 주요 기능

| 탭 | 기능 |
|---|---|
| 홈 | 시간대별 인사, 오늘 걸음 수(선셋 링), 거리/칼로리/스트릭, SUP 잔액·에너지, 루트 카드, 주간 차트(목표선 포함) |
| 워킹 | 포그라운드 워킹 세션(시작/일시정지/종료), 실시간 거리·걸음·시간·칼로리, 예상 적립, 세션 정산 카드, 화면 잠금 |
| 리워드 | SUP 지갑 히어로 카드(등급/잔액), 누적 적립·사용 요약, 적립 원장, GIWA 지갑 연동(준비 중) |
| 프로필 | 회원 카드, 스니커즈 레벨·능력치·업그레이드 진행률, 하루 목표 슬라이더, 앱 정보 |
...
- **UI**: Jetpack Compose + Material 3 (Cream & Sunset 라이트 테마), Navigation Compose
